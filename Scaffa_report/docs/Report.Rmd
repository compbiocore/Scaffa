...
* Used MACS2 (V 2.1.2) for peak calling. 
* Used the input libraries (H20 and LH20) as controls, while the IgG and HO1 libraries were considered treatments.
...
* This required creating a TxDb which was done using GenomicFeatures (V1.34.3) (`makeTxDbFromGFF`).
* Look [here](https://bioconductor.org/packages/devel/bioc/vignettes/GenomicFeatures/inst/doc/GenomicFeatures.pdf) for more information about TxDb objects.
* We based the TxDb on this [GFF3](ftp://ftp.ensembl.org/pub/release-95/gff3/mus_musculus).

## Part 15: Testing for significance of peaks
* Plotted the distribution of q-value scores across the permissive peaks (Part 13).
* The qvalue is a corrected p-value score. The FDR cutoff is 0.05 and it uses the Benjamini-Hochberg procedure.

### GroupA q-value distribution
![](assets/HO1_A_-log10qValue_histogram.png)

### GroupB q-value distribution
![](assets/HO1_B_-log10qValue_histogram.png)

* The x-axis is the -log10 q-value reported by MACS2.
* The red line indicates where the -log10 of the q-value is equal to 2 (corresponds to a q-value of ~0.01).
* Some of the peaks have -log10 q-values larger than this value.
...
![](assets/permissive_HO1_peaks_plotAnnoPie.png)

**plotDisttoTSS**

![](assets/permissive_HO1_peaks_plotDistToTSS.png)

* These figures suggest that most of the HO1-specific peaks are either less than 1KB from a promoter/TSS or in distal intergenic regions (10-100kb from promoter/TSS).

## Part 17: Peaks per chromosome

* Plotted histograms to get a better understanding of the distribution of peaks across chromosomes.

![](assets/peaks_per_chromosome_HO1_specific.png)
* The peaks are not evenly distributed across all chromosomes.

![](assets/peak_width_per_chromosome_HO1_specific.png)
* Although there is some variation, peaks are generally less than 1kb in width.
